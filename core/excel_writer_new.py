...
class ExcelWriter:
    ...
    def __init__(self, image_mode="mixed"):
        ...
        self.styles = {
            ...
            'description_font': Font(name='Consolas', size=12),
    ...
    def _generate_rebar_visual(self, rebar):
        ...
        shape_type = rebar.get('type', None)
        ...
        # 檢查是否支援圖片生成
        if shape_type and self.graphics_available and self.image_mode in ['image', 'mixed']:
            try:
                # 使用新的統一方法生成圖片
                if shape_type == 'type10':
                ...
                if image:
                    ...
                    print(f"🔍 生成 {shape_type} 鋼筋圖片: {temp_img_path}")
                    return temp_img_path
                    
            except Exception as e:
                print(f"⚠️ 生成 {shape_type} 鋼筋圖片失敗: {e}")
    ...
    def write_footer(self, row):
        ...
        cell.value = (f"生成時間：{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | "
                     f"圖示功能：已啟用，支援 {len(self.graphics_manager.get_supported_types())} 種鋼筋類型")
